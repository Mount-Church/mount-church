import "./assets/css/stylesAnimateVisionMissionAndValues.css";
import { useState } from "react";
import { initialTabs as tabs } from "./FuncionAnimateVisionMissionAndValues";
import { motion, AnimatePresence } from "framer-motion";

export default function AnimateVisionMissionAndValues() {
    const [selectedTab, setSelectedTab] = useState(tabs[0]);

    return (
        <div className="windowAnimateVisionMissionAndValues">
            <nav>
                <ul className="ulWindowAnimateVisionMissionAndValues">
                    {tabs.map((item) => (
                        <li
                            key={item.label}
                            className={item === selectedTab ? "selectedTabAnimated liWindowAnimateVisionMissionAndValues" : "liWindowAnimateVisionMissionAndValues"}
                            onClick={() => setSelectedTab(item)}
                        >
                            {`${item.icon} ${item.label}`}
                            {item === selectedTab ? (
                                <motion.div className="underlineWindowAnimateVisionMissionAndValues" layoutId="underlineWindowAnimateVisionMissionAndValues" />
                            ) : null}
                        </li>
                    ))}
                </ul>
            </nav>
            <main>
                <AnimatePresence mode="wait">
                    <motion.div
                        key={selectedTab ? selectedTab.label : "empty"}
                        initial={{ y: 10, opacity: 0 }}
                        animate={{ y: 0, opacity: 1 }}
                        exit={{ y: -10, opacity: 0 }}
                        transition={{ duration: 0.2 }}
                    >
                        {
                            selectedTab ?
                                <div>
                                    <h2 className="titleTextAnimateVisionMissionAndValues">
                                        {selectedTab.animateVisionMissionAndValuesTitle}
                                    </h2>
                                    <p className="textAnimateVisionMissionAndValues"
                                        dangerouslySetInnerHTML={{ __html: selectedTab.description }} />
                                </div>
                                : "😋"
                        }

                    </motion.div>
                </AnimatePresence>
            </main>
        </div>
    );
}
